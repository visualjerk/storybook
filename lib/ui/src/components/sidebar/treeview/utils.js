...
const toList = memoize(50)(dataset => Object.values(dataset));
...
export const getMains = memoize(50)(dataset => toList(dataset).filter(m => m.depth === 0));
const getMainsKeys = memoize(50)(dataset => getMains(dataset).map(m => m.id));
...
const exactMatch = filter => i =>
  (i.kind && i.kind.includes(filter)) ||
  (i.name && i.name.includes(filter)) ||
  (i.parameters && i.parameters.fileName && i.parameters.fileName.toString().includes(filter)) ||
